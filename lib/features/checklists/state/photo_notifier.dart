...
final photoRepositoryProvider = Provider<PhotoRepository>((ref) {
  ...
  return PhotoRepository(db, storage);
});
...
class PhotoNotifier
  ...
  Future<List<Photo>> build(({int itemId, int checklistId}) args) async {
    ...
    _photoRepository = ref.read(photoRepositoryProvider);

    return _photoRepository.getPhotosByItem(_itemId);
  }

  Future<void> addPhoto(String path) async {
    final current = state.value ?? [];

    final photoId = await _photoRepository.addPhoto(
      itemId: _itemId,
      path: path,
    );
